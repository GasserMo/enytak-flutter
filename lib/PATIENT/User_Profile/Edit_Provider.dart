...
class EditProvider extends StatefulWidget {
  ...
  const EditProvider({required this.labData, Key? key}) : super(key: key);
...
class _EditProviderState extends State<EditProvider> {
  ...
  void initState() {
    ...
    nameController = TextEditingController(text: widget.labData['name']);
    emailController = TextEditingController(text: widget.labData['email']);
    phoneNumberController =
        TextEditingController(text: widget.labData['phone_number']);
    addressController = TextEditingController(text: widget.labData['address']);
    bioController = TextEditingController(text: widget.labData['bio']);
    certificationsController =
        TextEditingController(text: widget.labData['certifications']);
    yearsOfExperienceController = TextEditingController(
        text: widget.labData['years_of_experience'].toString());
    averageRatingController = TextEditingController(
        text: widget.labData['average_rating'].toString());
    descriptionController =
        TextEditingController(text: widget.labData['description']);
    cityController = TextEditingController(text: widget.labData['city']);
    regionController = TextEditingController(text: widget.labData['region']);
    degreeController = TextEditingController(text: widget.labData['degree']);
    classificationController =
        TextEditingController(text: widget.labData['classification']);
    isActive = widget.labData['is_active'] ?? false;
    verificationStatus = widget.labData['verification_status'] ?? "pending";
  }
  ...
  Future<void> updateProviderData() async {
    ...
    final String apiUrl =
        'http://164.92.111.149/api/labs/${widget.labData['id']}/';
    ...
    final request = http.MultipartRequest('PUT', Uri.parse(apiUrl))
      ...
      ..fields.addAll({
        ...
        'user': widget.labData['user'].toString(),
  ...
  Future<void> pickServiceImage() async {
    try {
      ...
      if (pickedFile != null) {
        setState(() {
          _selectedServiceImage = File(pickedFile.path);
        });

        Fluttertoast.showToast(msg: "Service image selected successfully.");
      }
    } catch (e) {
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    if (widget.labData['license_document'] != null)
                      GestureDetector(
                        onTap: () => _viewFileInApp(
                            context, widget.labData['license_document']),
                        child: Card(
                          elevation: 5,
                          child: ListTile(
