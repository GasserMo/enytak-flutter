...
class _DiagnosisPageState extends State<DiagnosisPage> {
  ...
  void initState() {
    ...
    _loadUserType();
    _fetchMedicalFile(); // Load user type
  }
...
// Fetch medical record, category, and check if diagnosis already exists
  Future<void> _fetchMedicalRecordAndCategory() async {
    ...
    try {
      ...
      final medicalRecordsResponse = await http.get(
        Uri.parse('http://164.92.111.149/api/medical-records/'),
      );
      ...
      final diagnosisResponse = await http.get(
        Uri.parse('http://164.92.111.149/api/diagnoses/'),
      );
  ...
  Future<void> submitDiagnosis() async {
    ...
    final url = Uri.parse('http://164.92.111.149/api/diagnoses/');
  ...
  Future<void> _fetchMedicalFile() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (results != null) {
        results = data['results']
            .where(
                (result) => result['medical_record'] == _medicalRecord!['id'])
            .toList();
