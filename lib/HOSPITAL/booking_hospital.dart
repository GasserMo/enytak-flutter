...
class ScheduleHospitalScreen extends StatefulWidget {
  final int hospitalId; // Hospital ID passed from ProviderDetailsScreen
  final int serviceId;
  final String price;
  final int userId;
  const ScheduleHospitalScreen(
  ...
  @override
  _ScheduleHospitalScreenState createState() => _ScheduleHospitalScreenState();
}

class _ScheduleHospitalScreenState extends State<ScheduleHospitalScreen> {
  ...
  Future<void> _fetchAvailableSlots() async {
    if (widget.userId != null && selectedDate != null) {
      ...
      final url = Uri.parse(
          'http://164.92.111.149/api/users/${widget.userId}/availabilities/?page=1');
  ...
  Future<void> _onBookAppointment() async {
    if (selectedDate != null &&
      ...
      final appointmentData = {
        ...
        "hospital": widget.hospitalId, // Hospital ID passed to the screen
        "lab": null, // Set to null if lab is not involved
        "services": [
          widget.serviceId
